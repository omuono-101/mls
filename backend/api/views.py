...
class UnitViewSet(viewsets.ModelViewSet):
    ...
    def content_status(self, request, pk=None):
        """
        Lightweight endpoint so student UI can show:
        - has_approved_content
        - has_pending_approval_content (drafts exist but not approved)
        Does NOT leak content body/files of unapproved items.
        """
        unit = self.get_object()
        user = request.user

        # Students can only query their enrolled units
        if user.role == 'Student':
            ...
            if not enrolled:
                return Response({'detail': 'Not enrolled in this unit.'}, status=status.HTTP_403_FORBIDDEN)

        approved_lessons = Lesson.objects.filter(unit=unit, is_approved=True).exists()
        approved_resources = Resource.objects.filter(lesson__unit=unit, is_approved=True).exists()
        approved_assessments = Assessment.objects.filter(unit=unit, is_approved=True).exists()

        pending_lessons = Lesson.objects.filter(unit=unit, is_approved=False).exists()
        pending_resources = Resource.objects.filter(lesson__unit=unit, is_approved=False).exists()
        pending_assessments = Assessment.objects.filter(unit=unit, is_approved=False).exists()

        return Response({
            'unit_id': unit.id,
            'has_approved_content': bool(approved_lessons or approved_resources or approved_assessments),
            'has_pending_approval_content': bool(pending_lessons or pending_resources or pending_assessments),
    ...
    def generate_cats(self, request, pk=None):
        ...
        if cat_frequency and cat_frequency > 0:
            ...
            return Response({'error': 'Invalid CAT frequency configured for this unit.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if assessment_id:
            try:
                ...
                for enc in enrollments:
                    ...
                    status_val = 'Present' if sub else (att.status if att else 'Absent')
...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def upcoming_deadlines(self, request):
        ...
        upcoming_cats = Assessment.objects.filter(
            ...
            due_date__lte=(now + datetime.timedelta(days=upcoming_days)).date()
        ).select_related('unit', 'unit__course_group').order_by('due_date')

        upcoming_intakes = Intake.objects.filter(
            end_date__gte=now.date(),
            end_date__lte=(now + datetime.timedelta(days=upcoming_days)).date()
        )
